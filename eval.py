...
# tagged yaml objects
from experiment import Experiment, Structure, TrainSettings, ValidationSettings
from trainer import Trainer
from training.checkpoint import Checkpoint
from training.learning_rate import ConstantLearningRate, FileMonitorLearningRate, PriorityLearningRate
...
def main():
    ...
    parser.add_argument(
        "--start_iter", type=int, help="Begin counting iterations starting from this value (should be used with resume)"
    )
    parser.add_argument(
        "--start_epoch", type=int, help="Begin counting epoch starting from this value (should be used with resume)"
    )
    ...
    parser.add_argument("--no-verbose", action="store_true", help="show verbose info")
    ...
    parser.add_argument("--speed", action="store_true", dest="test_speed", help="Test speed only")
    parser.add_argument("--dest", type=str, help="Specify which prediction will be used for decoding.")
    parser.add_argument(
        "--debug",
        action="store_true",
        dest="debug",
...
class Eval:
    def __init__(self, experiment, args, cmd=dict(), verbose=False):
        ...
        self.model_path = cmd.get("resume", os.path.join(self.logger.save_dir(model_saver.dir_path), "final"))
    ...
    def report_speed(self, model, batch, times=100):
        ...
        self.logger.info(
            "Params: %s, Inference speed: %fms, FPS: %f"
            % (str(sum(p.numel() for p in model.parameters() if p.requires_grad)), time_cost * 1000, 1 / time_cost)
        )
