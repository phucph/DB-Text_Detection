...
def project_point_to_segment(x, u, v, axis=0):
    ...
    near_u = np.less_equal(
        np.linalg.norm(u - p, axis=axis, keepdims=True), np.linalg.norm(v - p, axis=axis, keepdims=True)
    )
...
class MakeSimpleDetectionData(Configurable):
    ...
    def get_regions_coords(self, w, h, polys, heights, shrink):
        ...
        regions_coords = [region.coords[:, ::-1] for region in regions] + [np.zeros((0, 2), "int32")] * (
