...
class _ResizeImage:
    ...
    def get_image_size(self, image):
        ...
        if self.mode == "keep_ratio":
            width = max(width, int(height / image.shape[0] * image.shape[1] / 32 + 0.5) * 32)
        if self.mode == "pad":
            width = min(width, max(int(height / image.shape[0] * image.shape[1] / 32 + 0.5) * 32, 32))
        return height, width
...
class ResizeData(_ResizeImage, DataProcess):
    ...
    def process(self, data):
        ...
        data[self.box_key][:, :, 0] = data[self.box_key][:, :, 0] * new_width / width
        data[self.box_key][:, :, 1] = data[self.box_key][:, :, 1] * new_height / height
        return data
