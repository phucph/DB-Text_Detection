...
class MakeCenterMap(DataProcess):
    ...
    def gaussian(self, points, height, width):
        index_x, index_y = np.meshgrid(np.linspace(0, width, width), np.linspace(0, height, height))
        ...
        result = np.reciprocal(2 * np.pi * width / self.sigma_ratio * height / self.sigma_ratio) * np.exp(
            -0.5
            * (
                np.square((index_x - mu_x) / width * self.sigma_ratio)
                + np.square((index_y - mu_y) / height * self.sigma_ratio)
            )
        )

        result = result / np.maximum(
            result.max(axis=1, keepdims=True).max(axis=2, keepdims=True), np.finfo(np.float32).eps
        )
    ...
    def sample_gaussian(self, points, height, width):
        ...
        for index in range(canvas.shape[0]):
            point = points[index]
            canvas[index, point[1], point[0]] = 1.0
            if point.sum() > 0:
                fi.gaussian_filter(
                    canvas[index],
