...
class InstanceDiceLoss(DiceLoss):
    ...
    def __init__(self, threshold=0.3, iou_thresh=0.2, reduction=None, max_regions=100, eps=1e-6):
        ...
        for batch_index, gt_instance_maps in enumerate(gt_label_maps):
            ...
            for gt_instance_map in gt_instance_maps:
                ...
                if instance_loss is None:
                    instance_loss = self._compute(
                        pred[batch_index][0], gt[batch_index][0], mask[batch_index] * gt_instance_map
                    )
            ...
            for instance_index in mask_not_matched:
                single_loss = self.replace_or_add(
                    single_loss,
                    self._compute(
                        pred[batch_index][0], gt[batch_index][0], mask[batch_index] * pred_instance_maps[instance_index]
                    ),
                )
