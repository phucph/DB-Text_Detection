...
class Trainer:
    ...
    def init_model(self):
        return self.structure.builder.build(self.device, self.experiment.distributed, self.experiment.local_rank)
    ...
    def train(self):
        ...
        while True:
            ...
            for batch in train_data_loader:
                ...
                if (
                    self.experiment.validation
                    and self.steps % self.experiment.validation.interval == 0
                    and self.steps > self.experiment.validation.exempt
                ):
                ...
                self.train_step(model, optimizer, batch, epoch=epoch, step=self.steps)
                if self.logger.verbose:
                    torch.cuda.synchronize()
                self.logger.report_time("Forwarding ")

                self.model_saver.maybe_save_model(model, epoch, self.steps, self.logger)
    ...
    def validate(self, validation_loaders, model, epoch, step):
        ...
        for name, loader in validation_loaders.items():
            if self.experiment.validation.visualize:
                ...
                self.logger.images(os.path.join("vis", name), vis_images, step)
            else:
